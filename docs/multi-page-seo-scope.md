...
**Status:** Signed off (Adrian, 2026-09-26) — landed as the plan of record.
...
**no schema or template code changes ship from this doc.**

**Build is parked, not queued.** Per Adrian's 2026-09-26 decision, the build
(schema extension → per-service pages → …, §9) does not get filed as follow-up
issues now. It is recorded in ops `docs/ai/PARKED.md` ("Multi-page local SEO
build") with **trigger: a client buys the multi-page SEO upsell**. When that
fires, file the ranked-rollout issues from §9 against this doc rather than
...
**Update (2026-09-26, checked against live issue bodies):** both #33 and #41
are now **closed as `not_planned`** (closed 2026-07-28), superseding the draft
below, which assumed they were still open and scoped a sequencing dependency
on them. Neither shipped: no FAQ section vocabulary landed from #33, and #41's
Keystatic prototype/decision doc was never built. That resolves the original
caveat ("confirm against the live issue bodies") but changes the sequencing
conclusion — recorded here rather than rewriting the original draft below,
which stays for its reasoning:

- **#33 (section kit) — not_planned, nothing shipped.** The "freeze #33's
  shape before scoping the per-service-page build issue" sequencing below no
  longer applies — there is nothing to wait on. If/when the multi-page build
  is unparked (§ above), `copy.faq` should be designed fresh at that time
  (still following the same closed-set-vocabulary principle #33 argued for),
  not blocked on a dependency that no longer exists.
- **#41 (Keystatic) — not_planned, nothing shipped.** The original "does not
  block on #41" conclusion still holds, now more simply: there is no Keystatic
  integration to design against. `pages.*.copy` ships as plain config fields
  if/when the build is unparked.

Original draft (2026-07-13), kept for its reasoning:
...
1. **Schema extension** (§7) — additive, low risk, unblocks everything else.
   No longer gated on #33 (closed `not_planned`, §6) — `copy.faq`'s shape
   should be designed fresh when this step is actually scoped.
